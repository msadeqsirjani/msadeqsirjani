...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    if request.status_code == 200:
        ...
        if response_data['data']['repository'] is None:
            # Repository doesn't exist or is private
            return 0
        if response_data['data']['repository']['defaultBranchRef'] is not None: # Only count commits if repo isn't empty
            return loc_counter_one_repo(owner, repo_name, data, cache_comment, response_data['data']['repository']['defaultBranchRef']['target']['history'], addition_total, deletion_total, my_commits)
        else: 
            return 0
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            ...
            except (TypeError, KeyError): # If the repo is empty or has no default branch
                data[index] = repo_hash + ' 0 0 0 0\n'
    with open(filename, 'w') as f:
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2000, 4, 27))
